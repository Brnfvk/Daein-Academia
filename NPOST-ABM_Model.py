...
def gomovingclass(movingclasstype):
    ...
    for i in range(0,len(student)):
        Yposition = 86.25
        if movingclasstype == "A":
        	movingclassnumber = Aclass[i]
        if movingclasstype == "B":
        	movingclassnumber = Bclass[i]
        if movingclasstype == "C":
        	movingclassnumber = Cclass[i]
        if movingclasstype == "D":
        	movingclassnumber = Dclass[i]
        if movingclasstype == "E":
        	movingclassnumber = Eclass[i]
        ...
        if movingclassnumber == 1:
            Xmovingclassposition.append(87.5*7)
        elif movingclassnumber == 2:
            Xmovingclassposition.append(87.5*6)
        elif movingclassnumber == 3:
            Xmovingclassposition.append(87.5*5)
        elif movingclassnumber == 4:
            Xmovingclassposition.append(87.5*4)
        elif movingclassnumber == 5:
            Xmovingclassposition.append(87.5*2)
        elif movingclassnumber == 6:
            Xmovingclassposition.append(87.5*1)
        elif movingclassnumber == 7:
            Xmovingclassposition.append(87.5*-0)
        elif movingclassnumber == 8:
            Xmovingclassposition.append(87.5*-1)
        elif movingclassnumber == 9:
            Xmovingclassposition.append(87.5*-2)
        elif movingclassnumber == 10:
            Xmovingclassposition.append(87.5*-3)
        elif movingclassnumber == 11:
            Xmovingclassposition.append(87.5*-5)
        elif movingclassnumber == 12:
            Xmovingclassposition.append(87.5*-6)
        movingXpos.append(Xmovingclassposition[i] - ((movingstudentnumber%5)*2)*87.5/11)
        if movingstudentnumber%5 == 0:
            movingXpos[i] = Xmovingclassposition[i] - (5*2)*87.5/11
        if movingclassnumber == 1 or movingclassnumber == 2:
            movingYpos.append(Yposition - (movingstudentnumber//5+1)*2*75/15)
        else:
            movingYpos.append(Yposition - (movingstudentnumber//5+1)*2*75/13)
        if movingstudentnumber%5 == 0: 
            if movingclassnumber == 1 or movingclassnumber == 2 : movingYpos[i] = Yposition - (movingstudentnumber//5)*2*75/15
            else:
                movingYpos[i] = Yposition - (movingstudentnumber//5)*2*75/13 
    aisle = []
    rightposition = 0
...
def period(Day, classperiod):
    if Day == "Tue" and classperiod == 7 or Day == "Thu" and classperiod == 1:
        gomovingclass("A")
    elif Day == "Mon" and classperiod == 3 or Day == "Wed" and classperiod == 5:
        gomovingclass("B")
    elif Day == "Wed" and classperiod == 2 or Day == "Thu" and classperiod == 4:
        gomovingclass("C")
    elif Day == "Tue" and classperiod == 1 or Day == "Thu" and classperiod == 6:
        gomovingclass("D")
    elif Day == "Mon" and classperiod == 5 or Day == "Fri" and classperiod == 3:
        gomovingclass("E")
    else:
        goclass()
def breaktimemove():
...
def studentsetting():
    for i in range(studentnum):
        student.append(i)
        if i <= 32:
            substudent.append(20100+i+1)
        elif i <= 65:
            substudent.append(20200+i-32)
        else:
            substudent.append(20000+((i-66)//29+3)*100+(i-66)%29+1)
        student[i] = t.Turtle()
...
def week():
   ...
   for day in Day:
      ...
      Daywriter.write("Day ",True)
      Daywriter.write(DayNum,True)
      initposition()
      ...
      for classtime in range (1, 8):
         if day != "Fri" or classtime != 7:
            ...
            writer.clear()
            if day == "Mon":
            	writer.write("Monday", True)
            if day == "Tue":
            	writer.write("Tuesday", True)
            if day == "Wed":
            	writer.write("Wednesday", True)
            if day == "Thu":
            	writer.write("Thursday", True)
            if day == "Fri":
            	writer.write("Friday", True)
            writer.write("   ", True)
            if classtime == 1: 
               writer.write(classtime, True)
               writer.write("st", True)
            elif classtime == 2: 
               writer.write(classtime, True)
               writer.write("nd", True)
            elif classtime == 3: 
               writer.write(classtime, True)
               writer.write("rd", True)
            else : 
               writer.write(classtime, True)
               writer.write("th", True)
            writer.write(" class", True)
            ...
            if classtime != 7:
               if day != "Fri" or classtime != 6:
                  ...
                  if day == "Fri":
                  	writer.write("Friday", True)
                  if classtime == 4:
                        writer.write("  Lunchtime", True)
                        movingstudent.clear()
                        movingrestriction()
                        lunchbreakmove()
                  else:
                        writer.write("  Breaktime", True)
                        movingstudent.clear()
                        movingrestriction()
                        breaktimemove()
      writer.clear()
      writer.goto(-400,200)
      writer.write("Day End")
      ...
      funcExposed()
      contact()
      for d in range(len(ContactTime)):
         ContactTime[d] = 0
      if DayNum == testday:
         for a in Confirmed:
            state[student.index(a)] = 10
            Confirmed.clear()
      statecheck()
      coloring()
      print("Exposed:",len(Exposed),"Infectious:",len(Infectious)+len(IMasking)+1,"Asymptomatic:",len(Asymptomatic)+len(AMasking))
      record()
      if DayNum == testday+1:
        break
def record():
...
def coloring():
    for sc in range(len(student)):
        if state[sc] == 0:
        	student[sc].color("Red")
        if state[sc] == 1:
        	student[sc].color("Lime")
        if state[sc] == 2:
        	student[sc].color("Lime")
        if state[sc] == 3:
        	student[sc].color("Green")
        if state[sc] == 4:
        	student[sc].color("Green")
        if state[sc] == 5:
        	student[sc].color("Blue")
        if state[sc] == 6:
        	student[sc].color("Orange")
        if state[sc] == 7:
        	student[sc].color("Orange")
        if state[sc] == 8:
        	student[sc].color("Magenta")
        if state[sc] == 9:
        	student[sc].color("Magenta")
        if state[sc] == 10:
        	student[sc].hideturtle()
        if state[sc] == 11:
        	student[sc].hideturtle()
        if state[sc] == 12:
        	student[sc].color("Pink")
        if state[sc] == 13:
        	student[sc].hideturtle()
def statecheck():
    Susceptible.clear()
